...
lemma collinear_0_proportional':
  ...
  shows "Mobius_gyrovector_space.collinear x 0\<^sub>m v \<longleftrightarrow> (\<exists> k::real. to_complex x = k * (to_complex v))"
  unfolding Mobius_gyrovector_space.collinear_def gyroplus_PoincareDisc_def gyroinv_PoincareDisc_def
  using assms
proof transfer
  fix v x
...
lemma collinear_0_proportional:
  shows "Mobius_gyrovector_space.collinear x 0\<^sub>m v \<longleftrightarrow> v = 0\<^sub>m \<or> (\<exists> k::real. to_complex x = k * (to_complex v))"
  by (metis Mobius_gyrovector_space.collinear_def collinear_0_proportional')
...
proof (cases "y = 0\<^sub>m")
  case True
  then show ?thesis
    using Mobius_gyrovector_space.between_xyx[of "0\<^sub>m" x]
    by auto
next
  case False
  then show ?thesis
