...
interpretation MobiusGyroTarskiIso: ElementaryTarskiHyperbolicIso p_congruent p_between \<phi> cong\<^sub>m Mobius_gyrovector_space.between
proof
  show "bij \<phi>"
...
next
  fix x y z
  show "Mobius_gyrovector_space.between (\<phi> x) (\<phi> y) (\<phi> z) \<longleftrightarrow> p_between x y z"
  proof-
    ...
    have "Mobius_gyrovector_space.between (\<phi> x) (\<phi> y) (\<phi> z) \<longleftrightarrow> 
          Mobius_gyrovector_space.between 0\<^sub>m (?f (\<phi> y)) (?f (\<phi> z))"
      by (metis Mobius_gyrovector_space.between_translate Mobius_gyrovector_space.translate_def gyro_left_inv gyrozero_PoincareDisc_def)
...
interpretation MobiusGyroTarski: ElementaryTarskiHyperbolic cong\<^sub>m Mobius_gyrovector_space.between
